...
public class CompleteVision217 
{
		...
		public static void main( String[] args)
		{
			...
			int topH, bottomH, topS, bottomS, topV, bottomV; 
			...
			VideoCapture captureHG = new VideoCapture();
			...
			try
			{
				captureGear.open("STRING FOR NEW CAMERA"); 
			}
			catch(Exception e)
			{
				captureGear = null;
			...
			while (true)  
			{
				...
				if (piVals.getBoolean("HGVision Enabled", false) && captureHG.isOpened())
				{
					...
					topH = (int) piVals.getNumber("Top H", 0); 
					topS = (int) piVals.getNumber("Top S", 0);
					topV = (int) piVals.getNumber("Top V", 0);
					bottomH = (int) piVals.getNumber("Bottom H", 0);
					bottomS = (int) piVals.getNumber("Bottom S", 0);
					bottomV = (int) piVals.getNumber("Bottom V", 0);
							
					Core.inRange(hsvConvert, new Scalar (bottomH, bottomS, bottomV), new Scalar (topH, topS, topV), hsv); //those two Scalar values are the the max and min HSV values respectively. Those were determined in GRIP. 
				...
				if (piVals.getBoolean("Gear Vision Enabled", false) && captureGear.isOpened())
				{
					System.out.println("TRIAL NUMBER " + oneCount); 
